...
    showChat();
    chatBox.innerHTML = '';
    // --- NEW: Clear history on new chat session ---
    chatHistory = [];
...
    if (userName) {
        startChat(userName);
        appendMessage(`Hello, ${userName}! How can I help you today?`, 'bot');
    } else {
        showHomepage();
    }
}
...
    
    // --- NEW: Store the user's message in the chat history ---
...
                // --- NEW: Send the entire chat history with each request ---
                history: chatHistory
            }),
        });
...
        }
        // --- NEW: Store the AI's response in the chat history ---
        chatHistory.push({ sender: 'ai', message: data.response });
    } catch (error) {
        chatBox.removeChild(loadingMessage.parentElement);
